...
## Защита кода (ЗАДАЧА 3: Обфускация)

### PyArmor обфускация

Начиная с усиления защиты лицензирования, критичные модули обфусцируются через PyArmor перед сборкой PyInstaller.

**Обфусцируемые модули:**
- `main.py` — точка входа с проверкой лицензии
- `licensing/` — вся система лицензирования
- `ui/widgets/license_dialog.py` — UI активации

**НЕ обфусцируются:**
- `core/` — детекторы знаков (не содержат бизнес-логики лицензий)
- `processing/` — обработка видео
- Все остальные модули приложения

### Стабильность для Delta-обновлений

**КРИТИЧНО**: PyArmor создаёт runtime-папку `pyarmor_runtime_XXXXXX` рядом с обфусцированными модулями.
Для стабильных delta-обновлений используется флаг `-i` при обфускации пакетов, который размещает
runtime-файлы ВНУТРИ самого пакета (например, `licensing/pyarmor_runtime_000000/`).

Это гарантирует:
- Фиксированное местоположение runtime-файлов между релизами
- Delta-манифест корректно отслеживает изменения runtime-файлов
- Нет "мёртвых" `pyarmor_runtime_*` папок после обновлений

**Реализация**: см. `scripts/build/obfuscate_licensing.py` — использование `pyarmor gen -i` для пакетов.

### Автоматическая обфускация при сборке

Скрипт `scripts\build\prepare_release.bat` автоматически выполняет обфускацию:

```batch
REM Шаг [2.5/6]: Обфускация лицензионных модулей
python scripts\build\obfuscate_licensing.py

REM Шаг [3/6]: Сборка из обфусцированного кода
cd build\obfuscated
pyinstaller signer.spec --noconfirm
```

### Ручная обфускация (для тестирования)

```cmd
# 1. Установка PyArmor
pip install pyarmor>=9.0.0

# 2. Обфускация модулей
python scripts\build\obfuscate_licensing.py

# 3. Сборка из обфусцированного кода
cd build\obfuscated
pyinstaller signer.spec --noconfirm
cd ..\..
```
...
После сборки проверьте, что:

1. **PyArmor runtime включён:**
   ```cmd
   dir /s dist\Signer\_internal\pyarmor_runtime_*
   ```
   Должна существовать папка `pyarmor_runtime_XXXXXX`

2. **Обфусцированные модули работают:**
   ```cmd
   dist\Signer\Signer.exe
   ```
   Приложение должно запускаться и показывать диалог лицензии

3. **Декомпиляция защищена:**
   
   Попытка декомпиляции обфусцированного `.pyc` через `uncompyle3` должна:
   - Либо завершиться с ошибкой
   - Либо выдать нечитаемый/искажённый код

### Примечания о PyArmor

- **Community Edition (Free):** Достаточно для базовой обфускации, но имеет ограничения
- **Pro Edition:** Расширенная защита runtime, anti-debugging, более сложная обфускация
- **Licen

se:** Уточните у владельца проекта наличие Pro-лицензии PyArmor

Подробнее: https://pyarmor.readthedocs.io/

---

## Возможные будущие улучшения защиты

### Nuitka (полная компиляция в C)

**Преимущества:**
- Компиляция Python → C → нативный код
- Значительно сложнее реверс-инжиниринга
- Потенциально лучшая производительность

**Недостатки:**
- Сложная настройка для тяжёлых стеков (PyQt6 WebEngine, torch, ultralytics)
- Высокий риск сломать рефлексию/динамическую загрузку плагинов
- Требует многодневной стабилизации сборки

**Статус:** Зафиксировано как research-спайк на будущее (оценка: 3-5 дней)

### PyArmor Pro

**Преимущества:**
- Advanced runtime protection
- Anti-debugging механизмы
- Более сильная обфускация

**Требуется:** Покупка Pro-лицензии PyArmor

### Вынесение бизнес-логики на сервер

**Концепция:** Финальная пост-обработка GeoJSON выполняется на сервере как облачный сервис

**Преимущества:**
- Пиратство теряет смысл — без сервера продукт неполнофункционален
- Дополнительные возможности (облачное хранение, синхронизация)

**Недостатки:**
- Архитектурно большое изменение
- Требует постоянной работы сервера
- Зависимость от интернета для полной функциональности

**Статус:** Не в рамках текущего промпта, требует отдельного проектирования
