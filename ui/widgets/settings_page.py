...
class SettingsPage(QWidget):
    ...
    def _check_tile_url_mismatch(self):
        ...
        if tile_type == "vector":
            ...
            if not (has_z and has_x and has_y):
                blocking_errors.append(
                    ...
                    "Исправьте URL, заменив конкретные числа на {z}, {x}, {y}."
                )
        
        # Проверка на "оборванный" URL (заканчивается на & ? = без последующего параметра)
        if tile_type == "vector":
            if tile_url.endswith(('&', '?', '=')):
                blocking_errors.append(
                    "❌ URL заканчивается на некорректный символ ('&', '?', или '=')\n\n"
        ...
        if blocking_errors:
            ...
            QMessageBox.critical(
                self,
                "Невозможно сохранить настройки карты",
                combined_msg + "\n\n❌ Сохранение отменено. Исправьте ошибки и попробуйте снова."
            )
            # Блокируем сохранение через исключение
            raise ValueError("Vector tile URL validation failed: missing placeholders or malformed URL")
        
        # ПРЕДУПРЕЖДЕНИЯ (не блокируют сохранение, но информируют)
        warning_msgs = []
        ...
        if tile_type == "vector":
            ...
            if suspicious_patterns:
                warning_msgs.append(
                    ...
                    "• Нет пробелов\n"
                    "• Нет двойных амперсандов &&\n\n"
                    "Некорректный URL может привести к ошибкам загрузки карты."
                )
