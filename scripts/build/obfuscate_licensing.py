...
Обфусцирует модуль licensing/ и main.py с помощью PyArmor перед сборкой релиза.
...
if sys.platform == 'win32':
    try:
        ...
        pass  # Если не получилось, fallback в log() и error()
...
def log(message: str):
    ...
    try:
        ...
        # Fallback для Windows консолей без UTF-8
...
def copy_unobfuscated_modules():
    """Копирует необфусцированные модули в build/obfuscated/ для сборки."""
    log("Копирование необфусцированных модулей...")
    
    # Модули которые НЕ нужно обфусцировать, но нужны для работы
    modules_to_copy = [
        "app",
        "core",
        "configs",
    ...
    for module_name in modules_to_copy:
        src_dir = ROOT / module_name
        if not src_dir.exists() or not src_dir.is_dir():
            log(f"  ⚠️ Пропускаю {module_name} (не найден)")
            continue
        
        dst_dir = OBFUSCATED_OUTPUT_DIR / module_name
        
        # Копируем весь модуль
        try:
            shutil.copytree(src_dir, dst_dir, dirs_exist_ok=True)
            log(f"  ✅ {module_name}/ скопирован")
        except Exception as e:
            error(f"  ❌ Ошибка копирования {module_name}: {e}")
            sys.exit(1)
...
def obfuscate_with_pyarmor():
    ...
    # Ищем pyarmor.exe в venv
    venv_dir = Path(sys.executable).parent
    pyarmor_exe = venv_dir / "pyarmor.exe"
    
    if not pyarmor_exe.exists():
        # Fallback: глобальный pyarmor
        pyarmor_exe = shutil.which("pyarmor")
        if not pyarmor_exe:
            error(f"PyArmor не установлен! Установите: pip install pyarmor>=9.0.0")
            error(f"Ожидался: {venv_dir / 'pyarmor.exe'}")
            sys.exit(1)
    
    try:
        result = subprocess.run(
            [str(pyarmor_exe), "--version"],
            capture_output=True,
            text=True,
            check=True,
            cwd=ROOT
        ...
        error(f"PyArmor не установлен! Установите: pip install pyarmor>=9.0.0")
        error(f"PyArmor executable: {pyarmor_exe}")
        error(f"Error: {e}")
        sys.exit(1)
    ...
    # 3. Копируем необфусцированные модули
    copy_unobfuscated_modules()
