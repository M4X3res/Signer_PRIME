...
**ВАЖНО (ЗАДАЧА 7):** Если на сервере настроена email-интеграция, при покупке через Stripe клиент автоматически получит лицензионный ключ на email. Для ручного создания через admin API email не отправляется — ключ выводится в stdout.
